...
class BatchResultPDFGenerator:
    ...
    def format_measurements(self, measurements_data):
        ...
        if "measuring_mode" in measurements:
            text_lines.append(f"Measurement mode: {measurements['measuring_mode']}")
        else:
            text_lines.append(f"Measurement mode: none")
...
def get_top_category(result):
    ...
    try:
        ...
        return 'other'  # Put malformed entries at the end
