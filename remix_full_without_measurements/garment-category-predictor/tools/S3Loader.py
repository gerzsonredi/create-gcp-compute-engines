...
class S3Loader():
    ...
    def load_category_model(self):
        ...
        REGION                      = self.__config.get("REGION", "")
        MODEL_NAME_SUBCATEGORY      = self.__config.get("MODEL_NAME_SUBCATEGORY", "")
        MODEL_NAME_SUBCATEGORY_PATH = self.__config.get("MODEL_NAME_SUBCATEGORY_PATH", "")
        LABEL_MAPS_PATH             = self.__config.get("LABEL_MAPS_PATH", "")
        ...
        try:
            response   = s3_client.get_object(Bucket=BUCKET_NAME, Key=MODEL_NAME_SUBCATEGORY_PATH)   # s3:GetObject required
            buffer     = io.BytesIO(response["Body"].read())            # keep it in RAM
            state_dict = torch.load(buffer, map_location=map_location)
            
            response_maps = s3_client.get_object(Bucket=BUCKET_NAME, Key=LABEL_MAPS_PATH)
            maps = json.loads(response_maps["Body"].read().decode('utf-8'))
